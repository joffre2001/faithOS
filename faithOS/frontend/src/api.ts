...
export const api = {
  ...
  uploadChurchLogoFor: (id: number, file: File) => uploadImage(`/churches/${id}/logo`, file),
  churchLogo: (version?: number) => apiUrl(`/churches/current/logo${version ? `?v=${version}` : ""}`),
  churchLogoFor: (id: number, version?: number) => apiUrl(`/churches/${id}/logo${version ? `?v=${version}` : ""}`),
