...
const nav = [
  ...
  { label: "Giving", icon: CircleDollarSign },
  { label: "Attendance", icon: ClipboardCheck },
  { label: "Care", icon: MessageSquare },
...
function Login({
  ...
  const setupText = {
    en: [
      ...
      "This account will become your church administrator.",
      "Church name",
      "First name",
      "Last name",
      "Phone",
      ...
      "Creating workspace…",
      "Create church and administrator",
      "Back to sign in",
      "Set up FaithOS for the first time →",
    ],
    fr: [
      "Configuration initiale",
      "Créez l’espace de votre église",
      "Ce compte deviendra l’administrateur de votre église.",
      "Nom de l’église",
      "Prénom",
      "Nom",
      "Téléphone",
      ...
      "Création en cours…",
      "Créer l’église et l’administrateur",
      "Retour à la connexion",
      "Configurer FaithOS pour la première fois →",
    ],
    "pt-BR": [
      "Configuração inicial",
      "Crie o espaço da sua igreja",
      "Esta conta será a administradora da sua igreja.",
      "Nome da igreja",
      "Nome",
      "Sobrenome",
      "Telefone",
      ...
      "Voltar para entrar",
      "Configurar o FaithOS pela primeira vez →",
    ],
    ht: [
      "Premye konfigirasyon",
      "Kreye espas legliz ou a",
      "Kont sa a pral administratè legliz ou a.",
      "Non legliz",
      "Prenon",
      "Siyati",
      "Telefòn",
      "Omwen 8 karaktè",
      "Ap kreye espas la…",
      "Kreye legliz ak administratè",
      "Retounen konekte",
      "Konfigire FaithOS pou premye fwa →",
    ],
  }[language];
  const registrationText = {
    en: [
      "Church registration",
      "Create your church workspace",
      "Create an administrator account for your church.",
      "Creating account…",
    ...
    fr: [
      ...
      "Créez l’espace de votre église",
      "Créez un compte administrateur pour votre église.",
      "Création du compte…",
      "Créer le compte de l’église",
      "Créer un compte d’église →",
    ],
    "pt-BR": [
      "Cadastro da igreja",
      "Crie o espaço da sua igreja",
      "Crie uma conta de administrador para sua igreja.",
      "Criando conta…",
      "Criar conta da igreja",
      "Criar uma conta para a igreja →",
    ],
    ht: [
      "Enskripsyon legliz",
      "Kreye espas legliz ou a",
      "Kreye yon kont administratè pou legliz ou a.",
      "Ap kreye kont la…",
      "Kreye kont legliz",
  ...
  async function initialize(event: React.FormEvent) {
    ...
    setError("");
    try {
      const session = await api.registerChurch(setup);
      onLogin(session);
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message
          : "Unable to create the church account."
      );
    } finally {
      setLoading(false);
    }
  ...
  async function requestReset(event: React.FormEvent) {
    ...
    try {
      ...
      setRecoveryMessage(
        "If an active account matches that email, a reset code has been sent."
      );
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Unable to request a reset code."
      );
    } finally {
      setLoading(false);
    }
      ...
      <section className="login-story">
        ...
        <div className="story-copy">
          <span className="eyebrow light">
            Church life, beautifully organized
          </span>
          <h1>
            Make space for
            <br />
          ...
          <p>
            One peaceful place for your people, ministries, giving, and every
            moment that brings your church together.
          </p>
        </div>
        <div className="verse">
      ...
      <section className="login-panel">
        ...
        {recoveryMode ? (
          <form
            ...
            onSubmit={recoverySent ? confirmReset : requestReset}
          >
            <span className="eyebrow">Account recovery</span>
            <h2>
              {recoverySent ? "Enter your reset code" : "Reset your password"}
            </h2>
            <p>
              {recoverySent
                ? recoveryMessage
          ...
          <form className="login-card setup-card" onSubmit={initialize}>
            <span className="eyebrow">{registrationText[0]}</span>
            <h2>{registrationText[1]}</h2>
            <p>{registrationText[2]}</p>
            <label>
              {setupText[3]}
              <input
                value={setup.churchName}
                onChange={(e) =>
                  setSetup({ ...setup, churchName: e.target.value })
                }
                required
              />
            </label>
            <div className="setup-name-row">
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSetup({ ...setup, firstName: e.target.value })
                  }
                  required
                />
              </label>
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSetup({ ...setup, lastName: e.target.value })
                  }
                  required
                />
              </label>
            ...
            <label>
              ...
              <input
                type="email"
                value={setup.email}
                onChange={(e) => setSetup({ ...setup, email: e.target.value })}
                required
              />
            </label>
            <label>
              ...
              <input
                value={setup.phone}
                onChange={(e) => setSetup({ ...setup, phone: e.target.value })}
              />
            </label>
            <label>
              Password
            ...
            {error && <div className="form-error">{error}</div>}
            <button className="primary full" disabled={loading}>
              {loading ? registrationText[3] : registrationText[4]}
            </button>
            <button
              type="button"
              className="text-button"
      ...
      <section className="login-story">
        ...
        <div className="story-copy">
          ...
          <p>
            ...
            entering your church workspace.
          </p>
        </div>
      </section>
      <section className="login-panel">
        ...
        <form className="login-card" onSubmit={submit}>
          ...
          <p>Welcome, {session.fullName}. Use at least 12 characters.</p>
          <label>
            Temporary password
            <input
              type="password"
...
function Sidebar({
  ...
  const visibleNav = nav.filter(
    (item) => isAdmin || !["People", "Giving", "Reports"].includes(item.label)
  );
  return (
    <>
      <aside className={`sidebar ${open ? "open" : ""}`}>
        ...
        <button className="church-switch">
          ...
          <span>
            <b>{churchName || "Your church"}</b>
            <small>Church workspace</small>
          </span>
          <ChevronDown size={16} />
        </button>
        <nav>
        ...
        <div className="sidebar-bottom">
          ...
          <div className="help-card">
            <HandHeart size={22} />
            <b>Need a hand?</b>
            <span>Visit the FaithOS help center.</span>
            <button
              onClick={() => {
                setScreen("Help");
                close();
...
function Dashboard({
  ...
  async function load() {
    ...
    setError("");
    try {
      const [eventData, givingData, ministryData] = await Promise.all([
        api.events(),
        isAdmin ? api.contributions() : Promise.resolve([]),
        api.ministries(),
      ]);
      setEvents(eventData);
      setContributions(givingData);
  ...
  const monthlyGiving = contributions
    .filter((item) => {
      const date = new Date(`${item.contributionDate}T12:00:00`);
      return (
  ...
  const activeMinistries = ministries.filter((item) => item.active).length;
  return (
    <>
      <header className="page-heading">
        ...
        <button className="primary" onClick={() => navigate("People")}>
          <Plus size={18} />
          Add person
        </button>
      </header>
      ...
      <section className="stats-grid">
        <article className="stat feature">
          <div>
            ...
            <strong>{isAdmin ? peopleCount ?? "—" : "Welcome"}</strong>
            <small>
              {isAdmin ? "Registered people" : "Your church workspace"}
            </small>
          </div>
          <Users size={34} />
        </article>
        <article className="stat">
          ...
          <small>On your church calendar</small>
          <button className="stat-link" onClick={() => navigate("Calendar")}>
            Open calendar →
          </button>
        </article>
        {isAdmin && (
          <article className="stat">
            <span>Giving this month</span>
            <strong>
              {loading
                ? "—"
                : monthlyGiving.toLocaleString("pt-BR", {
                    style: "currency",
                    currency: "BRL",
                  })}
            </strong>
            <small>{contributions.length} total contributions</small>
          </article>
        )}
        <article className="stat">
          <span>Active ministries</span>
          <strong>{loading ? "—" : activeMinistries}</strong>
          <small>{ministries.length} ministry teams</small>
        </article>
      </section>
      {!isAdmin && <PixDonation />}
      <section className="dashboard-grid">
        <article className="card">
          ...
          {!loading && upcoming.length === 0 && (
            <div className="dashboard-empty">
              ...
              <span>Create an event to see it here.</span>
            </div>
          )}
          {upcoming.map((item, index) => {
            ...
            return (
              <div className="event" key={item.id}>
                ...
                <span className="tag sage">{item.category || "Event"}</span>
              </div>
            );
          })}
        </article>
        <article className="card">
          <div className="card-title">
            <div>
              <span className="eyebrow">Church snapshot</span>
              <h3>Keep building</h3>
            </div>
          </div>
          <div className="care-row">
            ...
            <div>
              <b>{activeMinistries} active ministries</b>
              <span>Teams serving your community</span>
            </div>
            <button onClick={() => navigate("Ministries")}>View</button>
          </div>
          {isAdmin && (
            <div className="care-row">
              <span className="care-icon amber">
                <CircleDollarSign />
              </span>
              <div>
                <b>
                ...
                <span>Recorded generosity</span>
              </div>
              <button onClick={() => navigate("Giving")}>Review</button>
            </div>
          )}
          {isAdmin && (
            <div className="care-row">
              <span className="care-icon blue">
                <Users />
              </span>
              <div>
                <b>{peopleCount ?? 0} registered people</b>
                <span>Your church directory</span>
              </div>
              <button onClick={() => navigate("People")}>Connect</button>
            </div>
          )}
        </article>
      </section>
...
function People({
  ...
  async function save(event: React.FormEvent) {
    ...
    if (!validCpf(form.cpf)) {
      setFormError("Enter a valid CPF, including its verification digits.");
      return;
    }
    if (!editing && !session.churchId) {
      setFormError("Your account is not linked to a church.");
      return;
    }
    setSaving(true);
    setFormError("");
    try {
      ...
      setFormError(
        err instanceof Error ? err.message : "Unable to save this person."
      );
    } finally {
      setSaving(false);
    }
  ...
  async function toggle(user: User) {
    try {
      await api.setUserStatus(user.id, !user.active);
      await refresh();
    } catch (err) {
      setFormError(
  ...
  async function invite(user: User) {
    setActionMessage("");
    try {
      await api.inviteUser(user.id);
      setActionMessage(`Invitation sent to ${user.email}.`);
    } catch (err) {
      setActionMessage(
    ...
    <>
      <header className="page-heading">
        <div>
          ...
          <h1>People</h1>
          <p>Know your community and help everyone feel seen.</p>
        </div>
      ...
      <div className="toolbar">
        ...
        <button className="filter">{totalElements} people</button>
      </div>
      {actionMessage && (
        <div className="form-success directory-message">{actionMessage}</div>
      )}
      {error && (
        <div className="data-message error-state">
          <b>We couldn’t load your people.</b>
          <span>{error}</span>
        ...
        <section className="card people-card">
          ...
          {!loading && users.length === 0 && (
            <div className="data-message">
              <b>No people found</b>
              <span>Add the first person or change your search.</span>
            </div>
          )}
          {!loading &&
            users.map((user, index) => {
              ...
              const colors = ["peach", "mint", "lilac", "blue"];
              return (
                <div className="person-row" key={user.id}>
                  ...
                  <span>
                    <i className={`status ${!user.active ? "inactive" : ""}`} />
                  ...
                  <div className="person-actions">
                    ...
                    <button
                      ...
                      onClick={() => invite(user)}
                      aria-label={`Invite ${user.firstName}`}
                    >
                      <Mail size={14} />
                      Invite
                    </button>
      ...
      {selected && (
        ...
        >
          <section
            className="person-modal profile-modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            <dl className="profile-details">
              <div>
                <dt>Member ID</dt>
                <dd>{selected.memberCode || "Not provided"}</dd>
              </div>
              <div>
                <dt>Email</dt>
              ...
              <div>
                <dt>CPF</dt>
                <dd>{formatCpf(selected.cpf)}</dd>
              </div>
              <div>
                <dt>Role</dt>
                <dd>{selected.role.replaceAll("_", " ")}</dd>
              </div>
              <div>
                <dt>Church</dt>
                <dd>{selected.churchName || "Not assigned"}</dd>
              </div>
              <div>
                <dt>Status</dt>
              ...
              <div className="span-2">
                ...
                <dd>
                  {selected.emergencyContactName || "Not provided"} ·{" "}
                  {selected.emergencyContactPhone || "No phone"}
                </dd>
              </div>
            </dl>
            <div className="modal-actions">
...
function Placeholder({
  ...
  screen: Exclude<Screen, "Home" | "People">;
}) {
  const copy = {
    Calendar: [
      "Your shared rhythm",
      "Plan services, gatherings, and every moment in between.",
    ],
    Attendance: [
      "Gathering attendance",
      "Record worship and Bible-study participation.",
    ],
    Care: [
      "Member care",
      "Share absence reasons and communicate privately with church staff.",
    ],
    Giving: [
      "Generosity at a glance",
      "Track contributions and steward every gift with clarity.",
    ],
    Ministries: [
      "Teams with purpose",
    ...
    Notifications: [
      "Stay connected",
      "Share important updates with your church community.",
    ],
    Files: ["Shared files", "Keep important church documents organized."],
    Reports: [
      "Leadership reports",
      "See the health of your church in one place.",
    ],
    Settings: ["Workspace settings", "Manage your account and preferences."],
    Help: ["Help center", "Learn how to use FaithOS."],
  }[screen]!;
  return (
    <>
      <header className="page-heading">
      ...
      <section className="placeholder card">
        ...
        <h2>{screen} is ready for the next step</h2>
        <p>
          The navigation and visual system are in place. This workspace will
          connect to the next FaithOS API module.
        </p>
        <button className="secondary">Explore the layout</button>
      </section>
    </>
  );
}
...
function SettingsScreen({
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) =>
        setChurchError(
          err instanceof Error ? err.message : "Unable to load church settings."
        )
      );
  }, [session.churchId]);
  ...
  async function saveChurch(e: React.FormEvent) {
    ...
    try {
      ...
      setChurchError(
        err instanceof Error ? err.message : "Unable to update church settings."
      );
    } finally {
    ...
    <>
      <header className="page-heading">
        <div>
          ...
          <p>Manage your church, account security, and display preferences.</p>
        </div>
      </header>
      <section className="settings-grid">
        <article className="card settings-card">
          ...
          <div>
            ...
            <dl>
              <div>
                <dt>Role</dt>
                <dd>{session.role.replaceAll("_", " ")}</dd>
              </div>
              <div>
                <dt>Church</dt>
                <dd>{session.churchName || "Not assigned"}</dd>
              </div>
            </dl>
          </div>
        </article>
        <article className="card settings-card">
          ...
          <div>
            ...
            <p>Choose the language used throughout FaithOS.</p>
            <LanguageSelect language={language} onChange={setLanguage} />
          </div>
        </article>
        {church && (
          <form
            className="card password-settings church-settings"
            onSubmit={saveChurch}
          >
            <div className="settings-icon">
              <HeartHandshake />
            ...
            <div>
              <span className="eyebrow">Church profile</span>
              ...
              <div className="form-grid">
                ...
                <label>
                  ...
                  <input
                    ...
                    value={church.cnpj}
                    onChange={(e) =>
                      setChurch({ ...church, cnpj: e.target.value })
                    }
                  />
                </label>
                <label>
              ...
              {churchError && <div className="form-error">{churchError}</div>}
              {canEditChurch && (
                <button className="primary" disabled={churchSaving}>
                  {churchSaving ? "Saving…" : "Save church profile"}
                </button>
              )}
            </div>
          </form>
        ...
        {!church && churchError && (
          <div className="data-message error-state">
            <b>Church settings unavailable</b>
            <span>{churchError}</span>
          </div>
        )}
        <form className="card password-settings" onSubmit={changePassword}>
          <div className="settings-icon">
            <KeyRound />
          </div>
          <div>
...
function HelpScreen({
  ...
  const [support, setSupport] = useState({ page: "", expected: "", error: "" });
  const [copied, setCopied] = useState(false);
  const [supportSending, setSupportSending] = useState(false);
  const guides = [
    [
      "People",
      "Add accounts, assign roles, and activate or suspend access.",
      Users,
      admin ? "People" : "Home",
    ],
    [
      "Calendar",
      "Schedule worship services, meetings, and church events.",
      CalendarDays,
      "Calendar",
    ],
    [
      "Attendance",
      "Record worship and Bible-study attendance and review trends.",
      ClipboardCheck,
      "Attendance",
    ],
    [
      "Ministries",
      "Assign leaders and organize ministry members.",
      HeartHandshake,
      "Ministries",
    ],
    [
    ...
    [
      "Security",
      "Temporary passwords must be replaced on first sign-in.",
      ShieldCheck,
      "Settings",
    ],
  ] as const;
  async function sendSupport(e: React.FormEvent) {
    ...
    try {
      await api.submitSupport(support);
      setSupportOpen(false);
      setSupport({ page: "", expected: "", error: "" });
      window.alert("Your support request was sent successfully.");
    } catch (error) {
      window.alert(
  ...
  async function copyAddress() {
    await navigator.clipboard.writeText("obensonjoffre3@gmail.com");
    setCopied(true);
    ...
    <>
      <header className="page-heading">
        <div>
          ...
          <p>Quick guidance for the main FaithOS workflows.</p>
        </div>
      </header>
      <section className="help-grid">
        {guides.map(([title, description, Icon, target]) => (
          <article className="card help-guide" key={title}>
            <span>
              <Icon />
            </span>
            <h2>{title}</h2>
      ...
      <section className="card support-panel">
        <div>
          ...
          <div>
            ...
            <p>
              Include the page name, what you expected, and the exact error
              message when requesting support.
            </p>
            <small>obensonjoffre3@gmail.com</small>
          </div>
        </div>
          ...
          >
            ...
            <div className="form-grid">
              <label className="span-2">
                ...
                <input
                  required
                  placeholder="For example: People"
                  value={support.page}
                  onChange={(e) =>
                    setSupport({ ...support, page: e.target.value })
                  }
                />
              </label>
              <label className="span-2">
                ...
                <textarea
                  required
                  rows={3}
                  value={support.expected}
                  onChange={(e) =>
                    setSupport({ ...support, expected: e.target.value })
                  }
                />
              </label>
              <label className="span-2">
                What happened or which error appeared?
                <textarea
                  required
                  rows={4}
                  value={support.error}
                  onChange={(e) =>
                    setSupport({ ...support, error: e.target.value })
                  }
                />
              </label>
            </div>
            <div className="modal-actions">
              ...
              <button className="primary">Open email draft</button>
            </div>
          </form>
        </div>
      )}
...
function App() {
  ...
  const setLanguage = (next: Language) => {
    localStorage.setItem("faithos_language", next);
  ...
  async function refreshUsers() {
    ...
    setUsersError("");
    try {
      const result = await api.users(usersPage, 10, usersSearch);
      setUsers(result.content);
      setUsersTotal(result.totalElements);
  ...
  }, [session?.email, usersPage, usersSearch]);
  ...
  if (sessionLoading)
    return (
      <main className="login-shell">
        <section className="login-panel">
          <div className="login-card">Loading FaithOS…</div>
        </section>
      </main>
    );
  if (!session)
    return (
      <Login onLogin={signedIn} language={language} setLanguage={setLanguage} />
    );
    ...
    <div className="app-shell">
      ...
      <main className="main">
        ...
        <div className="content">
          {screen === "Home" ? (
            <Dashboard
              peopleCount={usersError ? null : usersTotal}
              firstName={session.fullName.split(" ")[0]}
              navigate={setScreen}
              isAdmin={isAdmin}
              language={language}
            />
          ) : screen === "People" && isAdmin ? (
            <People
              users={users}
            ...
            <CalendarScreen role={session.role} />
          ) : screen === "Attendance" ? (
            <AttendanceScreen role={session.role} />
          ) : screen === "Care" ? (
            <CareScreen role={session.role} userId={session.id} language={language} />
          ) : screen === "Notifications" ? (
            <NotificationsScreen role={session.role} />
          ) : screen === "Files" ? (
            <FilesScreen role={session.role} />
          ) : screen === "Settings" ? (
            <>
              <SettingsScreen
                session={session}
          ...
          ) : screen === "Reports" && isAdmin ? (
            <ReportsScreen />
          ) : screen === "Giving" && isAdmin ? (
            <GivingScreen />
          ) : (
            <MinistriesScreen role={session.role} userId={session.id} />
          )}
        </div>
      </main>
    </div>
